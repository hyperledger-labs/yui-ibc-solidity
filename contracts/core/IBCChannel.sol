...
library IBCChannel {
    ...
    function channelOpenInit(IBCHost host, IBCMsgs.MsgChannelOpenInit memory msg_) public returns (string memory) {
        host.onlyIBCModule();
    ...
    function channelOpenTry(IBCHost host, IBCMsgs.MsgChannelOpenTry memory msg_) public returns (string memory) {
        host.onlyIBCModule();
        ConnectionEnd.Data memory connection;
        require(msg_.channel.connection_hops.length == 1, "connection_hops length must be 1");
        connection = mustGetConnection(host, msg_.channel);
    ...
    function channelOpenAck(IBCHost host, IBCMsgs.MsgChannelOpenAck memory msg_) public {
        host.onlyIBCModule();
    ...
    function channelOpenConfirm(IBCHost host, IBCMsgs.MsgChannelOpenConfirm memory msg_) public {
        host.onlyIBCModule();
        Channel.Data memory channel;
        ConnectionEnd.Data memory connection;
    ...
    function channelCloseInit(IBCHost host, IBCMsgs.MsgChannelCloseInit memory msg_) public {
        host.onlyIBCModule();
        Channel.Data memory channel;
        ConnectionEnd.Data memory connection;
    ...
    function channelCloseConfirm(IBCHost host, IBCMsgs.MsgChannelCloseConfirm memory msg_) public {
        host.onlyIBCModule();
        Channel.Data memory channel;
        ConnectionEnd.Data memory connection;
    ...
    function recvPacket(IBCHost host, IBCMsgs.MsgPacketRecv calldata msg_) external {
        ...
        if (channel.ordering == Channel.Order.ORDER_UNORDERED) {
            ...
            uint64 nextSequenceRecv =
                host.getNextSequenceRecv(msg_.packet.destination_port, msg_.packet.destination_channel);
            require(
                nextSequenceRecv > 0 && nextSequenceRecv == msg_.packet.sequence,
                "packet sequence != next receive sequence"
            );
            host.setNextSequenceRecv(
                msg_.packet.destination_port, msg_.packet.destination_channel, nextSequenceRecv + 1
            );
        } else {
    ...
    function getCounterpartyHops(IBCHost host, Channel.Data memory channel)
        internal
        view
    ...
    function mustGetConnection(IBCHost host_, Channel.Data memory channel_)
        private
        view
    ...
    function mustGetChannel(IBCHost host_, string memory portId_, string memory channelId_)
        private
        view
        returns (Channel.Data memory)
    {
